...
function Disc({
  ...
  useFrame((_, delta) => {
    ...
    speed.current = THREE.MathUtils.damp(speed.current, isPlaying ? PLAY_SPEED : 0, 1.8, delta);
    if (spinRef.current) {
  ...
  return (
    <group ref={spinRef} position={[0, 0.045, 0]}>
      <mesh>
        <cylinderGeometry args={[1.02, 1.02, 0.035, 96]} />
    ...
    <group position={[1.4, 0.03, -0.55]}>
      ...
      <group ref={armRef}>
        <mesh position={[-0.55, 0.025, 0]} rotation={[0, 0, Math.PI / 2]}>
          ...
          <meshStandardMaterial color="#2f2f2f" roughness={0.4} metalness={0.5} />
        </mesh>
...
export const VinylScene = ({ audioRef, currentSong, index }: SectionProps) => {
  ...
  useFrame((_, delta) => {
    ...
    if (platterGroup) {
      ...
      platterGroup.position.y = THREE.MathUtils.damp(platterGroup.position.y, targetY, 5, delta);
    }
  });
  ...
  return (
    <group ref={rootRef} position={[0, -0.2, 0]}>
      <group rotation={[0.45, 0, 0]}>
        <RoundedBox args={[3.0, 0.32, 2.3]} radius={0.06} smoothness={4} position={[0, -0.16, 0]}>
          <meshStandardMaterial color="#3b2a20" roughness={0.65} metalness={0.05} />
        </RoundedBox>

        <group ref={platterGroupRef}>
          <mesh>
            <cylinderGeometry args={[1.15, 1.15, 0.05, 72]} />
            <meshStandardMaterial color="#c8c8ca" roughness={0.3} metalness={0.85} />
          </mesh>
          <Disc audioRef={audioRef} coverImage={currentSong.cover} />
        </group>

        <Tonearm audioRef={audioRef} />
...
export const VinylHtml = ({ audioRef, onNextSong, onPrevSong, currentSong }: SectionProps) => (
  <div className="exp-content" style={{ paddingTop: '8vh' }}>
    ...
    <div style={{ display: 'flex', gap: '0.75rem' }}>
      <button className="exp-btn" onClick={onPrevSong} aria-label="Vorheriger Song">
        ‹
      </button>
      <button
